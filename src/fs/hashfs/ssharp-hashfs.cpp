...
entry_list SSHARP_HASHFS_DLL ssharp::fs::hashfs::loadFile(const string& filename)
{
	...
	for (uint32_t c = 0; c < header.entries_count; c++)
	{
		...
		list.push_back(is_directory ? directory_obj(content) : basic_obj(content));
	}
	return list;
}
